...
class SQLEventFetcher(tableName: String, headVersionFetcher: SQLHeadVersionFetcher) extends EventFetcher {
  def fetchEventsFromDB(connection: Connection, version: Long = 0, batchSize: Option[Int] = None): EventPage = {
    ...
    val last = headVersionFetcher.fetchCurrentVersion(connection)
...
class SQLHeadVersionFetcher(tableName: String) {
