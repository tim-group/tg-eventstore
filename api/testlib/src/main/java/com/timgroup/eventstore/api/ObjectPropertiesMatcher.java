...
public class ObjectPropertiesMatcher<T> extends TypeSafeDiagnosingMatcher<T> {
    private final List<Matcher<T>> properties = new ArrayList<>();

    public static <T, R> ObjectPropertiesMatcher<T> objectWith(PropertyToMatch<T, R> property, R expectedValue) {
        ObjectPropertiesMatcher<T> matcher = new ObjectPropertiesMatcher<>();
        return matcher.and(property, expectedValue);
    }

    public <R> ObjectPropertiesMatcher<T> and(PropertyToMatch<T, R> property, R expectedValue) {
        return andMatching(property, is(expectedValue));
    }

    public <R> ObjectPropertiesMatcher<T> andMatching(PropertyToMatch<T, R> property, Matcher<R> expectedValue) {
