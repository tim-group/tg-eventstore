...
class HeapyCurrentBatchWriter {
    ...
    public HeapyCurrentBatchWriter(BatchingPolicy batchingPolicy,
    ...
    public S3BatchObject prepareBatchForUpload() throws IOException {
        try (ByteArrayOutputStream gzippedByteArrayOutputStream = new ByteArrayOutputStream(8192);
             GZIPOutputStream gzipOutputStream = new GZIPOutputStream(gzippedByteArrayOutputStream, 8192)) {

            batch.stream()
                    .map(this::toProtobufsMessage)
                    .map(this::toBytesPrefixedWithLength)
                    .forEach(uncompressedBytes -> {
