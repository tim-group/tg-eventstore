...
public class S3Archiver {
    ...
    private Position convertPosition(Optional<Long> positionInArchive) {
        ...
        return liveEventSource.positionCodec().deserializePosition(String.valueOf(maxPositionInArchive));
    }
    ...
    private final class ArchiveStalenessComponent extends Component {
        ...
        public Report getReport() {
            ...
            Optional<Long> livePosition = lastEventInLiveEventStore()
                    ...
                    .map(liveEventSource.positionCodec()::serializePosition)
                    .map(Long::valueOf);

            boolean isStale = batchingPolicy.isStale(maxPositionInArchive, lastEventInLiveEventStore(), liveEventSource.positionCodec());
    ...
    private Long positionFrom(ResolvedEvent eventFromLiveEventSource) {
        return Long.parseLong(liveEventSource.positionCodec().serializePosition(eventFromLiveEventSource.position()));
    }
